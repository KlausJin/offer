        ...
        for (int i = 0; i < map.size(); i++) {
            ...
            if (data == null) {                                     //查找对业务员隐藏的属性
                ...
                if (data.get("attr_id").equals("-1")) {     //attrid为-1的显示不启用
                    ...
                    bean.f_attrBeans = new AttrModel().getBeanByAttrId(data.get("spare_id"), offer_id, product_id, cat_id);
                list.add(bean);
            }
        }
        return list;
        ...
        if (res == null || res.get("attr_id").equals("-1")) {    //spareid为空的或者attrid为-1的进入循环
            ...
            if (cat_id.equals("32")) {
                if (map.get("price").equals("-2.0000")) {
                    HashMap<String, String> name_list = materialModel.getMaterial(map.get("name"));
                    if (name_list == null) {
                        map.replace("price", "-3");
                    } else {
                        map.replace("price", name_list.get("price"));
